...
$atomic_js = document.querySelectorAll('script.atomic-js'), $atomic_jscode = document.querySelectorAll('pre.language-javascript > code');
atomic_htmlsnippet = [], atomic_jsnippet = [];
...
for (var i = 0, l = $atomic_js.length; i < l; i++) {
  ...
  atomic_jsnippet.push($atomic_js[i].innerHTML.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'));
}
