(function(w) {
  ...
  let $atomic_html       = document.querySelectorAll('pre.language-html code'),
  ...
  for(let i = 0, l = $atomic_html.length; i < l; i++) {
  ...
  for(let i = 0, l = $atomic_js.length; i < l; i++) {
    atomic_jsnippet.push(
      $atomic_js[i].innerHTML.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    );
  }

  for(let i = 0, l = $atomic_jscode.length; i < l; i++) {
